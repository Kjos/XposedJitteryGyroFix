...
public class GyroscopeNoiseFilter implements IXposedHookLoadPackage {
    ...
    private static void antiJitterValues(float[] values, float[][] medianValues, float[] prevValues) {
    	// Note about values[]:
    	...
    	// Init arrays
    	int filter_size = medianValues.length;
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) throws Throwable {
        ...
        try {
            ...
            XposedBridge.hookAllMethods(sensorEQ, "dispatchSensorEvent", new
                    XC_MethodHook() {
                        ...
                        float prevValues[] = new float[medianValues[0].length]; // stores the previous sensor's values to restore them if needed
                        ...
                        private void changeSensorEvent(float[] values) {
                            ...
                            antiJitterValues(values, medianValues, prevValues);
                        }
        ...
        try {
        	...
        	Log.d(TAG, "Exception in Cardboard Head hook: "+t.getMessage());
            // Do nothing
        }
        */
        ...
        try {
            ...
            XposedBridge.hookAllMethods(cla, "getEyeView", new
                    XC_MethodHook() {
                        ...
                        protected void afterHookedMethod(MethodHookParam param) throws
                            ...
                            Log.d(TAG, "BABA1");
                            for (int i = 0;i<values.length;i++) {
                            	Log.d(TAG, "BABA values: "+i+" : "+values[i]);
                            	values[i] = 0.0f;
                            }
                            Log.d(TAG, "BABA2");
                        }
        	...
        	Log.d(TAG, "Exception in Cardboard Eye hook: "+t.getMessage());
            // Do nothing
        }
    }
